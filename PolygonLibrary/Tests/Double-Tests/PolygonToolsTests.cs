...
/// <summary>
///This is a test class for polygon tools and is intended
///to contain all polygon tools unit tests
///</summary>
[TestFixture]
      ...
      Assert.Multiple
        (
         () => {
           ...
           Assert.That
             (r.Contour[0], Is.EqualTo(p), "Turned rectangle, alpha = " + a + ", i = " + i + ": wrong point in the contour");
         }
        );
    }
  }
    ...
    for (int i = 0; i <= N; i++) {
      ...
      if (Tools.NE(a) && Tools.NE(a, Tools.PI / 2) && Tools.NE(a, Tools.PI)) {
        ...
        for (k = 1; k < r.Contour.Count; k++) {
          ...
          Assert.That
            (
             Tools.EQ(double.Abs(e1 * Vector2D.E1), cos) || Tools.EQ(double.Abs(e1 * Vector2D.E2), cos)
        ...
        Assert.That
          (r.Contour, Has.Count.EqualTo(2), "Turned rectangle, alpha = " + a + ", i = " + i + ": too many points in the contour");
      }
    }
  }
