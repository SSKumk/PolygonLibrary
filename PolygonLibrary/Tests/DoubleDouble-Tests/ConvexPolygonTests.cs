...
#region Data
	...
	public void RandomPoint2()
	{
		...
		Vector2D[]
			cornerPoints = Array.FindAll(rndPoints, p => ddouble.Abs(p.x) > 1 && ddouble.Abs(p.y) > 1),
			_01Points    = Array.FindAll(rndPoints, p => 0 < p.x && p.x < 1 && 0 < p.y && p.y < 1);

		ddouble cornPointsFreq = cornerPoints.Length * 7, _01PointsFreq = _01Points.Length * 14;
