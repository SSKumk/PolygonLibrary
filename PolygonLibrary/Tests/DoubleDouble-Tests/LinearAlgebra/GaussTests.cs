...
public class GaussTests {
  ...
  public void HundredSystem() {
    Matrix    A_             = GenNonSingularMatrix(100);
  ...
  private static void Check(ddouble[,] A, ddouble[] b, ddouble[] expectedResult, bool isSingular = false) {
    Assert.Multiple
      (
       () => {
         bool successNo  = GaussSLE.SolveNoChange(A, b, GaussSLE.GaussChoice.No, out ddouble[] resNo);
         bool successRow = GaussSLE.SolveNoChange(A, b, GaussSLE.GaussChoice.RowWise, out ddouble[] resRow);
         bool successCol = GaussSLE.SolveNoChange(A, b, GaussSLE.GaussChoice.ColWise, out ddouble[] resCol);
         bool successAll = GaussSLE.SolveNoChange(A, b, GaussSLE.GaussChoice.ColWise, out ddouble[] resAll);

         if (isSingular) {
           Assert.That(successNo, Is.EqualTo(false),  "GaussSLE.SolveNoChange should return true for GaussChoice.No");
           Assert.That(successRow, Is.EqualTo(false), "GaussSLE.SolveNoChange should return true for GaussChoice.RowWise");
           Assert.That(successCol, Is.EqualTo(false), "GaussSLE.SolveNoChange should return true for GaussChoice.ColWise");
           Assert.That(successAll, Is.EqualTo(false), "GaussSLE.SolveNoChange should return true for GaussChoice.ColWise");
         } else {
           Assert.That(successNo, "GaussSLE.SolveNoChange should return true for GaussChoice.No");
           Assert.That(successRow, "GaussSLE.SolveNoChange should return true for GaussChoice.RowWise");
           Assert.That(successCol, "GaussSLE.SolveNoChange should return true for GaussChoice.ColWise");
           Assert.That(successAll, "GaussSLE.SolveNoChange should return true for GaussChoice.ColWise");
