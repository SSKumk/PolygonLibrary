...
#region Auxiliary functions
  ...
  private Point GenConvexCombination(List<Point> points, GRandomLC? random = null) {
    List<ddouble> ws = new List<ddouble>();

    ddouble difA = 1;
    for (int i = 0; i < points.Count - 1; i++) {
      ddouble alpha = GenInner(random) * difA;
      ws.Add(alpha);
      difA -= alpha;
    }
    ws.Add(difA);
...
#region Main Generators
  ...
  private List<Point> Simplex(int simplexDim, out List<Point> pureSimplex, IEnumerable<int>? facesDim = null, int amount = 1) {
    List<Point> simplex = new List<Point> { new Point(new ddouble[simplexDim]) };

    for (int i = 0; i < simplexDim; i++) {
      ddouble[] v = new ddouble[simplexDim];
      v[i] = 1;
  ...
  private List<Point> Cube(int               cubeDim
    ...
    List<List<ddouble>> cube_prev = new List<List<ddouble>>();
    List<List<ddouble>> cube      = new List<List<ddouble>>();
    cube_prev.Add(new List<ddouble>() { 0 });
    cube_prev.Add(new List<ddouble>() { 1 });

    for (int i = 1; i < cubeDim; i++) {
      cube.Clear();

      foreach (List<ddouble> coords in cube_prev) {
        cube.Add(new List<ddouble>(coords) { 0 });
        cube.Add(new List<ddouble>(coords) { 1 });
      }
      cube_prev = new List<List<ddouble>>(cube);
    }

    List<Point> Cube = new List<Point>();

    foreach (List<ddouble> v in cube) {
    ...
    if (facesDim is not null) { // накидываем точки на грани нужных размерностей
      foreach (int dim in facesDim) {
        ...
        for (int i = 0; i < amount; i++) {
          ddouble[] point = new ddouble[cubeDim];
