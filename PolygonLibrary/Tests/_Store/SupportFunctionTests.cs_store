...
namespace Tests
{
	...
	public class SupportFunctionTests
	{
		...
		public void CreateSFTest1()
		{
			...
			NUnit.Framework.Assert.AreEqual(sf.Count, 4, "Wrong number of resultant vectors");

			for (int i = 0; i < 4; i++) {
				NUnit.Framework.Assert.IsTrue(sf[i].Equals(gps1_true[i]), "i = " + i);
			}
		}
		...
		public void CreateSFTest2()
		{
			...
			for (int i = 0; i < 4; i++) {
				NUnit.Framework.Assert.IsTrue(sf[i].Equals(gps2_true[i]), "i = " + i);
			}
		}
		...
		public void ComputeSFTest1()
		{
			...
			for (int i = 0; i < vs.Length; i++) {
				NUnit.Framework.Assert.IsTrue(Tools.EQ(sf.FuncVal(vs[i]), gs[i]), "i = " + i);
			}
		}
		...
		public void CrossPairsTest()
		{
			...
			NUnit.Framework.Assert.IsTrue(hasException, "No exception during crossing parallel lines");

			p = GammaPair.CrossPairs(g1, g3);
			NUnit.Framework.Assert.IsTrue(p.Equals(p13), "Bad crossing g1 and g3");

			p = GammaPair.CrossPairs(g3, g1);
			NUnit.Framework.Assert.IsTrue(p.Equals(p13), "Bad crossing g3 and g1");

			p = GammaPair.CrossPairs(g1, g4);
			NUnit.Framework.Assert.IsTrue(p.Equals(p14), "Bad crossing g1 and g4");

			p = GammaPair.CrossPairs(g2, g3);
			NUnit.Framework.Assert.IsTrue(p.Equals(p23), "Bad crossing g2 and g3");

			p = GammaPair.CrossPairs(g2, g3);
			NUnit.Framework.Assert.IsTrue(p.Equals(p24), "Bad crossing g21 and g3");

			p = GammaPair.CrossPairs(g3, g4);
			NUnit.Framework.Assert.IsTrue(p.Equals(p34), "Bad crossing g3 and g4");
		}
		...
		public void CombineSFTest()
		{
			...
			{
				NUnit.Framework.Assert.IsTrue(sf11[i].Normal.Equals(sf1[i].Normal), "sf11: " + i + "th normal is wrong");
				NUnit.Framework.Assert.IsTrue(Tools.EQ(sf11[i].Value, 2 * sf1[i].Value), "sf11: " + i + "th value is wrong");
			}

			for (i = 0; i < sf22.Count; i++)
			{
				NUnit.Framework.Assert.IsTrue(sf22[i].Normal.Equals(sf2[i].Normal), "sf22: " + i + "th normal is wrong");
				NUnit.Framework.Assert.IsTrue(Tools.EQ(sf22[i].Value, 2 * sf2[i].Value), "sf22: " + i + "th value is wrong");
			}
			...
			for (i = 0; i < sf12.Count; i++)
			{
				if (i % 2 != 0)
				{
					NUnit.Framework.Assert.IsTrue(sf12[i].Normal.Equals(sf1[i / 2].Normal), "sf12: " + i + "th normal is wrong");
					NUnit.Framework.Assert.IsTrue(Tools.EQ(sf12[i].Value, 3 * sf1[i / 2].Value), "sf12: " + i + "th value is wrong");
				}
				else
				{
					NUnit.Framework.Assert.IsTrue(sf12[i].Normal.Equals(sf2[i / 2].Normal), "sf12: " + i + "th normal is wrong");
					NUnit.Framework.Assert.IsTrue(Tools.EQ(sf12[i].Value, 2 * sf2[i / 2].Value), "sf12: " + i + "th value is wrong");
				}
			}
		}
		...
		public void FindTest1()
		{
			...
			for (k = 0; k < testVecs.Count; k++)
			{
				...
				NUnit.Framework.Assert.IsTrue(i == res[k, 0] && j == res[k, 1],
		...
		public void FindTest2()
		{
			...
			SupportFunction sf = new SupportFunction(ps);
			List<double> testAngles = new List<double> { -30.0, -15.0, 0.0, 15.0, 30.0, 60.0, 90.0, 180.0, 210.0, 270.0, 300.0};
			double[,] res = new double[,]
				{
