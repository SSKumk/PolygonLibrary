  ...
  where TConv : INumConvertor<TNum>
{
  ...
  public class ConvexPolytop
  {
      ...
      get
      {
        if (_HRep is null)
        {
          ...
          else
          {
            ...
            { // Если дано вершинное описаение, то прокатываем поарок.
              _HRep = GW.HRep;
            }
          }
        }
      ...
      get
      {
        if (_FL is null)
        {
          ...
          else
          {
            ...
            { // FL = null, VRep = null; Печаль ;( HRep --> VRep -(GW)-> FL
    ...
    public static HashSet<Vector> HRepToVRep_Naive(List<HyperPlane> HPs)
    {
      ...
      do
      { // Перебираем все сочетания из d элементов из набора гиперплоскостей
        gaussSLE.SetSystem(AFunc, bFunc, d, d, GaussSLE.GaussChoice.RowWise);
        gaussSLE.Solve();
        if (gaussSLE.GetSolution(out Vector point))
    ...
    public static HashSet<Vector> HRepToVRep_Geometric(List<HyperPlane> HPs)
    {
      ...
      // Этап 1. Поиск какой-либо вершины и определение индексов гиперплоскостей, которым она принадлежит
      // Наивная реализация
      ...
      Queue<(Vector, IEnumerable<HyperPlane>)> process = new Queue<(Vector, IEnumerable<HyperPlane>)>();
      process.Enqueue((Vs.First(), HPs.Where(hp => hp.Contains(Vs.First()))));

      // Обход в ширину
      while (process.TryDequeue(out var elem))
      {
        (var z, var Hz) = elem;
        ...
        do // перебираем "рёбра"
        {
          List<HyperPlane> edge = J
          LinearBasis HzLinSpace = new LinearBasis(Hz.Select(hp => hp.Normal));
          bool isEdge = true;


          // ищем направляющий вектор прямой, перпендикулярный линейному пространству Hz  
          Vector v;
          int i = 0;
          do
          {
            i++;
            v = Vector.OrthonormalizeAgainstBasis(Vector.MakeOrth(d, i), HzLinSpace.Basis);
          } while (v.IsZero && i <= d);
          Debug.Assert(i <= d, $"ConvexPolytop.HRepToVRep_Geometric (dim = {d}): Can't find vector e! That orthogonal to the HzLinSpace.");

          // проверяем вектор v
          bool firstNonZeroProduct = true;
          foreach (var hp in Hz)
          {
            ...
            if (firstNonZeroProduct)
            {
              ...
              {
                // Меняем v на противоположный, если скалярное произведение положительное
                v = -v;
              }
            ...
            {
              // Для всех последующих не нулевых произведений требуется, чтобы они были отрицательные
              if (Tools.LT(dotProduct))
              {
                isEdge = false;
                break;
              }
          ...
          if (isEdge)
          {
            ...
            TNum tNew = Tools.One / Tools.Zero;
            Vector zNew = Vector.Zero(d);
            foreach (var hp in HPs)
            {
              TNum ti = (hp.ConstantTerm - hp.Normal * z) / (hp.Normal * v);
              if (Tools.LE(ti) || Tools.GT(ti, tNew))
              {
                continue;
              }
              if (Tools.EQ(ti, tNew))
            ...
            process.Enqueue((zNew, Hz.Union(newHPs)));
