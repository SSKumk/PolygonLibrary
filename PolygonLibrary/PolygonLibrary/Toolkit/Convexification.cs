...
#region Two-dimensional convexification by means of the QuickHull algorithm
  ...
  public static List<Point2D> QuickHull2D(List<Point2D> swarm) {
    ...
    List<Point2D> positive = swarm.Where(p => Tools.GT(line[p])).ToList(), negative = swarm.Where(p => Tools.LT(line[p])).ToList();
...
#region Two-dimensional convexification by means of Graham’s scan algorithm
  ...
  public static List<Point2D> GrahamHull(IEnumerable<Point2D> swarmOrig) {
    ...
    SortedDictionary<double, Point2D> swarmDict      = new SortedDictionary<double, Point2D>(doubleComparer) { { -1.0, origin } };
  ...
  private static bool IsLeft(Point2D p1, Point2D p2, Point2D p3) { return Tools.GT((p2 - p1).Normalize() ^ (p3 - p1).Normalize()); }
#endregion
