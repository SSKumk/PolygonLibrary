...
#region Constructors
    ...
    public CauchyMatrix(Matrix nA, TNum nT, TNum ndt) {
      Debug.Assert
        (nA.Rows == nA.Cols, $"CauchyMatrix.Ctor: The given matrix should be square! Found rows = {nA.Rows} cols = {nA.Cols}");
...
#region The getting functions
    ...
    private void ComputeAt(TNum t) {
      ...
      KeyValuePair<TNum, Matrix> leftVal = new KeyValuePair<TNum, Matrix>(), rightVal = new KeyValuePair<TNum, Matrix>();
      ...
      try {
        leftVal    = _ms.GetReverseEnumerator(t).Current;
        leftExists = true;
      }
      catch {
        // ignored
      ...
      try {
        rightVal    = _ms.GetEnumerator(t).Current;
        rightExists = true;
      }
      catch {
        // ignored
