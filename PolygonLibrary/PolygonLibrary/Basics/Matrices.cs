...
#region Functions
  ...
  static Matrix MatrixDuplicate(Matrix matrix)
  {
    ...
    for (int i = 0; i < matrix.Cols; ++i) // Копирование значений
    for (int j = 0; j < matrix.Rows; ++j)
      result[i,j] = matrix[i,j];
    return new Matrix(result);
  }
  
  private Matrix MatrixDecompose(out int[] perm, out int toggle)
  {
    // Разложение LUP Дулитла. Предполагается,
    // что матрица квадратная.
    int        n      = Rows*Cols; // для удобства
    double[,] result = MatrixDuplicate(this);
    perm = new int[n];
  ...
  double[] HelperSolve(double[][] luMatrix,
                              double[]   b)
  {
    // Решаем luMatrix * x = b
