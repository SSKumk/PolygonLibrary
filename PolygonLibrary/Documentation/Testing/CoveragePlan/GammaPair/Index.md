...
| CrossPairs | `x` | Основные непараллельные сценарии перенесены напрямую; параллельный случай вынесен из обязательного минимума. |
...
- Параллельный случай `CrossPairs` не включён в обязательный минимум, потому что реализация всё ещё опирается на `Debug.Assert`, а не на стабильный runtime-`throw`.
