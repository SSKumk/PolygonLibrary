 ...
 * `game.md5hash` -- содержит в себе хешы: задачи, динамики, многогранника ограничений на управления первого игрока, многогранника ограничений на управления второго игрока, терминальных множеств. 
...
* На уровень ниже папка `ЧИСЛОВОЙ_ТИП` её имя совпадает с именем числового типа, с которым была запущена библиотека. Например, `System.double` или `DoubleDouble.ddouble`.
  * На уровень ниже папка `ТОЧНОСТЬ` её имя совпадает со значением парамера точности, с которым была запущена библиотека, в научной записи. Например, `1E-08` или `1E-16`.
    ...
    * Здесь лежит файл `.structure`, хранящий в себе записи вида: `D) = N sec. |f0| = N, |f1| = N, ..., |f{d-1}| = N`, где первое число это момент времени для которого считалось данное сечение моста, далее идёт $f$-вектор (количество $k$-граней).
