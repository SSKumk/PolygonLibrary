...
public partial class Geometry<TNum, TConv> where TNum : struct, INumber<TNum>, ITrigonometricFunctions<TNum>, IPowerFunctions<TNum>, IRootFunctions<TNum>,
  ...
  public class SupportFunction : List<GammaPair> {
    ...
    public static SupportFunction CombineFunctions(SupportFunction fa
      ...
      TNum angleA = fa[0].Normal.PolarAngle, angleB = fb[0].Normal.PolarAngle, oldAngle;

      suspiciousIndices?.Clear();
      suspiciousVectors?.Clear();

      while (Tools.LE(angleA, Tools.PI) || Tools.LE(angleB, Tools.PI)) {
      ...
      while (suspicious.Count > 0) {
        ...
        if (list[i].isValid && !CheckTriple(list[i].prev, i, list[i].next)) {
          ...
          if (Tools.LT(this[list[i].prev].Normal.PolarAngle, this[list[i].next].Normal.PolarAngle)) {
            ...
            da = Tools.Two * Tools.PI + this[list[i].next].Normal.PolarAngle - this[list[i].prev].Normal.PolarAngle;
          }

          if (Tools.GE(da, Tools.PI)) {
            return null;
          }
