...
namespace ParamReader
{
  ...
  public class ParamReader
  {
    ...
    public int ReadInt(string name, char term = ';')
    {
      ...
      int res;
      if (!int.TryParse(readData, out res))
    ...
    public double ReadDouble(string name, char term = ';')
    {
      ...
      double res;
      if (!double.TryParse(readData, NumberStyles.Any,  CultureInfo.InvariantCulture, out res))
        throw new Exception("Cannot convert data '" + data + "' read during parsing object '" + name + "' into double");
        ...
        switch (data[ind])
        {
          ...
          case '\\':
            switch (data[ind + 1])
            {
              ...
              case '"': readData += '"'; break;
              case 'n': readData += '\n'; break;
              case 'r': readData += '\r'; break;
              case 't': readData += '\t'; break;
              default:
                throw new Exception("Erroneous escape symbol '\\" + data[ind + 1] + "' when reading string object '" + name + "'");
            }
            ind += 2;
            break;
    ...
    private IEnumerable<T> ReadArrayRow<T>(string name, int elemQnt)
    {
      ...
      else
      {
        ...
        if (converter == null)
          throw new Exception("Read1DArray: strange type '" + elemType.FullName + "' of array elements - no conversion from a string!");
        ...
        for (int i = 0; i < elemQnt; i++)
        {
          ...
          if (!converter.IsValid(readData))
            throw new Exception("Read1DArray: cannot convert data '" + data + "' read during parsing object '" + name + "' into '" + elemType.FullName + "'");

          yield return (T)(object)converter.ConvertFrom(null, CultureInfo.InvariantCulture, readData);
        }
      }
    }
    #endregion
