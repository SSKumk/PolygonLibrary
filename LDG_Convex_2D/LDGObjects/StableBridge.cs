...
namespace LDGObjects
{
  ...
  public class StableBridge2D : List<TimeSection2D>, IComparable<StableBridge2D>
  {
    ...
    public StableBridge2D(string problemName, string shortProblemName, double nc, TubeType type)
      : base()
    {
    ...
    public StableBridge2D(ParamReader pr) : base()
    {
      ...
      if (ContentType != SignatureBridge
          && ContentType != Signature1stTube
          && ContentType != Signature2ndTube)
    ...
    public void WriteToFile(StreamWriter sw)
    {
      ...
      for (int i = 0; i < Count; i++)
      {
		...
		public int GetSectionIndex(double t)
		{
			return this.BinarySearchByPredicate(sec => Tools.GE(sec.t, t));
		}
